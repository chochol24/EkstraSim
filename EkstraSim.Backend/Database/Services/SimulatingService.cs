...
public partial class SimulatingService : ISimulatingService
{
    ...
    private readonly IDbContextFactory<EkstraSimDbContext> _contextFactory;
    ...
    public static async Task<SimulatingService> CreateAsync(IDbContextFactory<EkstraSimDbContext> dbFactory)
    {
        ...
        var leaguesResponse = await context.Leagues.ToListAsync();
        var matchesResponse = await context.Matches
            .Include(x => x.AwayTeam)
            .Include(x => x.HomeTeam)
            .ToListAsync();
        var teamsRespone = await context.Teams
    ...
    public async Task<IEnumerable<SimulatedMatchResult>> SimulateRound(List<Match> matchesToSimulate, int leagueId, int numberOfSimulations)
    {
        ...
        foreach (var match in matchesToSimulate)
        {
            ...
            if (numberOfSimulations > 1)
            {
                ...
                double homeFormFactor = CalculateTeamFormFactor(homeLastXMatches, match.HomeTeamId);
                double awayFormFactor = CalculateTeamFormFactor(awayLastXMatches, match.AwayTeamId);

                var homeLastFiveHomeMatches = matches
                ...
                var awayLastFiveAwayMatches = matches
                ...
                var lastFiveH2HMatches = matches
    ...
    public async Task SimulateRestOfTheSeason(int leagueId, int seasonId, int? currentRound = 0, int? numberOfSimulations = 1000, string? comments = "")
    {
        ...
        var simulatedLeague = new SimulatedFinalLeague
        {
            LeagueId = leagueId,
            SeasonId = seasonId,
            RoundBeforeSimulation = currentRound.Value,
            NumberOfSimulations = numberOfSimulations.Value,
            Comments = comments,
        ...
        var simulatedTeams = teamsRes.Select(t => new SimulatedTeamInFinalTable
        {
            TeamId = t.Id,
        ...
        var simulatedSeasonsBag = new ConcurrentBag<List<SimulatedTeamSeasonStats>>();
        ...
        for (int i = 0; i < numberOfSimulations; i++)
        {
            ...
            tasks.Add(Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    var results = await SimulateSeasonInMemory(leagueId, seasonId, currentRound.GetValueOrDefault());
                    simulatedSeasonsBag.Add(results);
        ...
        simulatedSeasons = simulatedSeasonsBag.ToList();
        ...
        await using (var context = await _contextFactory.CreateDbContextAsync())
        {
            context.SimulatedTeamInFinalTables.AddRange(simulatedTeams);
            simulatedLeague.Teams = simulatedTeams;
            context.Update(simulatedLeague);
            await context.SaveChangesAsync();
        }
    ...
    private async Task<List<SimulatedTeamSeasonStats>> SimulateSeasonInMemory(int leagueId, int seasonId, int startRound)
    {
        ...
        for (int round = startRound; round <= Constants.NumberOfRoundsEkstaklasa; round++)
        {
